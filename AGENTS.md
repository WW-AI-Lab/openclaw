...
- Scoped package release guardrail: `@ww-ai-lab/openclaw` releases must verify both scoped and legacy package-name code paths before publish. At minimum: confirm `docs/reference/templates/AGENTS.md` exists inside the packed tarball, verify runtime package-root discovery recognizes both `@ww-ai-lab/openclaw` and `openclaw`, and run one real Control UI chat smoke test after installing the packed/global build.
