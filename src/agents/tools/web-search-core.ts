...
const DEFAULT_OPENAI_SEARCH_BASE_URL = "https://dashscope.aliyuncs.com/compatible-mode/v1";
const DEFAULT_OPENAI_SEARCH_MODEL = "qwen-plus";
...
function resolveOpenAISearchBaseUrl(config?: OpenAISearchConfig): string {
  const fromConfig =
    config && "baseUrl" in config && typeof config.baseUrl === "string"
      ? config.baseUrl.trim()
      : "";
  return fromConfig || DEFAULT_OPENAI_SEARCH_BASE_URL;
}
...
async function runWebSearch(params: {
          ...
          : params.provider === "kimi"
            ...
            : params.provider === "metaso"
              ...
              : params.provider === "openai-search"
                ? `${params.openaiSearchBaseUrl ?? DEFAULT_OPENAI_SEARCH_BASE_URL}:${params.openaiSearchModel ?? DEFAULT_OPENAI_SEARCH_MODEL}:${String(params.openaiSearchEnableThinking ?? false)}:${params.openaiSearchSearchParam ?? "enable_search"}`
                : "";
  ...
  if (params.provider === "openai-search") {
    const openaiSearchModel = params.openaiSearchModel ?? DEFAULT_OPENAI_SEARCH_MODEL;
    const openaiSearchToolName = params.openaiSearchToolName ?? "openai-search";
    const { content, citations } = await runOpenAISearch({
      query: params.query,
      apiKey: params.apiKey,
      baseUrl: params.openaiSearchBaseUrl ?? DEFAULT_OPENAI_SEARCH_BASE_URL,
      ...
      : provider === "grok"
        ...
        : provider === "kimi"
          ...
          : provider === "gemini"
            ...
            : provider === "metaso"
              ? "Search the web using Metaso. Returns AI-synthesized answers with citations from Chinese web search."
